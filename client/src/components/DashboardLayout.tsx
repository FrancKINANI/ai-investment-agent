...
import { Activity, Bell, Bot, ChevronRight, ClipboardCheck, ClipboardList, Keyboard, Landmark, LayoutDashboard, LogOut, MessageSquareText, Moon, Settings2, ShieldAlert, Sun, UserRound, WalletCards, X } from "lucide-react";
import { useLocation } from "wouter";

const menuItems = [
  { icon: LayoutDashboard, label: "Mission Control", path: "/" },
  { icon: MessageSquareText, label: "Agent Room", path: "/chat" },
  { icon: ClipboardList, label: "Tasks", path: "/tasks" },
  { icon: ClipboardCheck, label: "Decision Desk", path: "/decisions" },
  { icon: WalletCards, label: "Portfolio", path: "/portfolio" },
  { icon: Activity, label: "Activity", path: "/activity" },
  { icon: Settings2, label: "Configure", path: "/settings" },
];

const routeLabels = [...menuItems, { icon: WalletCards, label: "Portfolio", path: "/wallets" }, { icon: WalletCards, label: "Portfolio", path: "/mandates" }, { icon: WalletCards, label: "Portfolio", path: "/platforms" }, { icon: WalletCards, label: "Portfolio", path: "/connections" }, { icon: ShieldAlert, label: "Security alerts", path: "/alerts" }];
...
function WorkspaceTopbar({
  ...
  return <header className="os-topbar"><div className="os-trail"><SidebarTrigger className="os-sidebar-trigger" aria-label={drawerLabel} title={shortcutLabel} /><span className="os-mobile-drawer-label" aria-live="polite">{drawerLabel}</span><span>ledgerline</span><b>{routeLabels.find((item) => item.path === location)?.label ?? "Owner controls"}</b></div><div className="os-topbar-status"><span><i /> agents ready</span><span className="os-mode-chip">simulation default</span>{isAuthenticated && alertCount > 0 && <button type="button" onClick={() => navigate("/alerts")} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", border: "1px solid var(--ll-line)", borderRadius: 5, background: "color-mix(in srgb, var(--ll-danger) 12%, var(--ll-surface))", color: "var(--ll-danger)", font: "8px 'DM Mono',monospace", cursor: "pointer" }} title={`${alertCount} unacknowledged security alerts`}><Bell size={12} /><b className="os-unread-badge" style={{ background: "var(--ll-danger)" }}>{alertCount > 9 ? "9+" : alertCount}</b></button>}<span className="os-shortcut-hint" title={`Navigation shortcut: ${primaryShortcutLabel(navigationShortcut)}`}><Keyboard size={12} /><kbd>⌘</kbd><kbd>{navigationShortcut.toUpperCase()}</kbd></span>{!isAuthenticated && <Button size="sm" onClick={startLogin}>Sign in</Button>}</div></header>;
}

function MobileWorkspaceShell({ children, location, navigate, isAuthenticated, navigationShortcut, alertCount }: { children: React.ReactNode; location: string; navigate: (path: string) => void; isAuthenticated: boolean; navigationShortcut: string; alertCount: number }) {
  const { isMobile, toggleSidebar } = useSidebar();
  const touchStart = useRef<{ x: number; y: number; interactive: boolean } | null>(null);

  return <div className="os-mobile-workspace" onTouchStart={(event) => { const touch = event.touches[0]; const target = event.target as HTMLElement; touchStart.current = { x: touch.clientX, y: touch.clientY, interactive: Boolean(target.closest("input, textarea, select, button, a, [contenteditable='true']")) }; }} onTouchEnd={(event) => { const start = touchStart.current; const touch = event.changedTouches[0]; touchStart.current = null; if (!isMobile || !start || start.interactive) return; const horizontal = touch.clientX - start.x; const vertical = Math.abs(touch.clientY - start.y); if (start.x <= 28 && horizontal >= 72 && vertical <= 72) toggleSidebar(); }}>
    <SidebarInset className="os-inset"><WorkspaceTopbar location={location} isAuthenticated={isAuthenticated} navigationShortcut={navigationShortcut} alertCount={alertCount} /><main className="os-main">{children}</main></SidebarInset>
  </div>;
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const alertsQuery = trpc.security.alerts.unacknowledgedCount.useQuery(undefined, { enabled: isAuthenticated, retry: false, refetchInterval: 15_000 });
  const alertCount = alertsQuery.data ?? 0;
