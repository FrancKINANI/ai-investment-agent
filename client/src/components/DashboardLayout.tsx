...
import { Activity, Bot, Cable, Landmark, LogOut, PanelLeft, Settings2, WalletCards } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider defaultOpen>
    <div className="os-layout">
      ...
      <SidebarInset className="os-inset"><header className="os-topbar"><div className="os-trail"><PanelLeft size={15} /><span>ledgerline</span><b>{menuItems.find((item) => item.path === location)?.label ?? "Workspace"}</b></div><div className="os-topbar-status"><span><i /> agents ready</span><span className="os-mode-chip">simulation default</span>{isAuthenticated ? <em>{user?.name ?? "Owner"}</em> : <Button size="sm" onClick={startLogin}>Sign in</Button>}</div></header><main className="os-main">{children}</main></SidebarInset>
    </div>
  </SidebarProvider>;
}
