...
function MobileWorkspaceShell({ children, location, navigate, isAuthenticated, navigationShortcut, unreadActivityCount }: { children: React.ReactNode; location: string; navigate: (path: string) => void; isAuthenticated: boolean; navigationShortcut: string; unreadActivityCount: number }) {
  const { isMobile, toggleSidebar } = useSidebar();
  const touchStart = useRef<{ x: number; y: number; interactive: boolean } | null>(null);
  const tabs = menuItems.filter((item) => item.path === "/" || item.path === "/chat" || item.path === "/activity");
  ...
  return <div className="os-mobile-workspace" onTouchStart={(event) => { const touch = event.touches[0]; const target = event.target as HTMLElement; touchStart.current = { x: touch.clientX, y: touch.clientY, interactive: Boolean(target.closest("input, textarea, select, button, a, [contenteditable='true']")) }; }} onTouchEnd={(event) => { const start = touchStart.current; const touch = event.changedTouches[0]; touchStart.current = null; if (!isMobile || !start || start.interactive) return; const horizontal = touch.clientX - start.x; const vertical = Math.abs(touch.clientY - start.y); if (start.x <= 28 && horizontal >= 72 && vertical <= 72) toggleSidebar(); }}>
    ...
    <nav className="os-mobile-tabs" aria-label="Primary mobile navigation">{tabs.map((item) => <button type="button" key={item.path} className={location === item.path ? "active" : ""} aria-current={location === item.path ? "page" : undefined} onClick={() => navigate(item.path)}><item.icon size={17} /><span>{item.label}</span>{item.path === "/activity" && unreadActivityCount > 0 && <b>{unreadActivityCount > 9 ? "9+" : unreadActivityCount}</b>}</button>)}</nav>
  </div>;
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider defaultOpen shortcut={preferences.shortcuts.navigation}>
    <div className="os-layout">
      ...
      <MobileWorkspaceShell location={location} navigate={navigate} isAuthenticated={isAuthenticated} navigationShortcut={preferences.shortcuts.navigation} unreadActivityCount={unreadActivityCount}>{children}</MobileWorkspaceShell>
    </div>
  </SidebarProvider>;
}
