...
function WorkspaceTopbar({
  ...
  return <header className="os-topbar"><div className="os-trail"><SidebarTrigger className="os-sidebar-trigger" aria-label={drawerLabel} title={shortcutLabel} /><span className="os-mobile-drawer-label" aria-live="polite">{drawerLabel}</span><span>ledgerline</span><b>{routeLabels.find((item) => item.path === location)?.label ?? "Owner controls"}</b></div><div className="os-topbar-status"><span><i /> agents ready</span><span className="os-mode-chip">simulation default</span><span className="os-shortcut-hint" title={`Navigation shortcut: ${primaryShortcutLabel(navigationShortcut)}`}><Keyboard size={12} /><kbd>⌘</kbd><kbd>{navigationShortcut.toUpperCase()}</kbd></span>{!isAuthenticated && <Button size="sm" onClick={startLogin}>Sign in</Button>}</div></header>;
}

function MobileWorkspaceShell({ children, location, navigate, isAuthenticated, navigationShortcut }: { children: React.ReactNode; location: string; navigate: (path: string) => void; isAuthenticated: boolean; navigationShortcut: string }) {
  ...
  return <div className="os-mobile-workspace" onTouchStart={(event) => { const touch = event.touches[0]; const target = event.target as HTMLElement; touchStart.current = { x: touch.clientX, y: touch.clientY, interactive: Boolean(target.closest("input, textarea, select, button, a, [contenteditable='true']")) }; }} onTouchEnd={(event) => { const start = touchStart.current; const touch = event.changedTouches[0]; touchStart.current = null; if (!isMobile || !start || start.interactive) return; const horizontal = touch.clientX - start.x; const vertical = Math.abs(touch.clientY - start.y); if (start.x <= 28 && horizontal >= 72 && vertical <= 72) toggleSidebar(); }}>
    <SidebarInset className="os-inset"><WorkspaceTopbar location={location} isAuthenticated={isAuthenticated} navigationShortcut={navigationShortcut} /><main className="os-main">{children}</main></SidebarInset>
  </div>;
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider defaultOpen shortcut={preferences.shortcuts.navigation}>
    <div className="os-layout">
      ...
      <MobileWorkspaceShell location={location} navigate={navigate} isAuthenticated={isAuthenticated} navigationShortcut={preferences.shortcuts.navigation}>{children}</MobileWorkspaceShell>
    </div>
  </SidebarProvider>;
}
