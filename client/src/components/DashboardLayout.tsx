...
function WorkspaceTopbar({
  ...
  return <header className="os-topbar"><div className="os-trail"><SidebarTrigger className="os-sidebar-trigger" aria-label={drawerLabel} title={shortcutLabel} /><span className="os-mobile-drawer-label" aria-live="polite">{drawerLabel}</span><span>ledgerline</span><b>{menuItems.find((item) => item.path === location)?.label ?? "Workspace"}</b></div><div className="os-topbar-status"><span><i /> agents ready</span><span className="os-mode-chip">simulation default</span><span className="os-shortcut-hint" title={`Navigation shortcut: ${primaryShortcutLabel(navigationShortcut)}`}><Keyboard size={12} /><kbd>⌘</kbd><kbd>{navigationShortcut.toUpperCase()}</kbd></span><button className="theme-toggle" type="button" onClick={toggleTheme} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}>{theme === "dark" ? <Sun size={15} /> : <Moon size={15} />}</button>{!isAuthenticated && <Button size="sm" onClick={startLogin}>Sign in</Button>}</div></header>;
}
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <SidebarProvider defaultOpen shortcut={preferences.shortcuts.navigation}>
    <div className="os-layout">
      ...
      <SidebarInset className="os-inset"><WorkspaceTopbar location={location} isAuthenticated={isAuthenticated} theme={theme} toggleTheme={safeToggleTheme} navigationShortcut={preferences.shortcuts.navigation} /><main className="os-main">{children}</main></SidebarInset>
    </div>
  </SidebarProvider>;
}
