  ...
  return <div className="workspace-page connections-page">
    ...
    <div className="connection-grid">{adapters.map((adapter) => {
      ...
      return <article className="connection-card" key={adapter.name}>
        ...
        <footer><small><ShieldCheck size={13} /> {connection ? "Paper adapter active" : adapter.docs}</small>{connection ? <Button variant="outline" onClick={() => toast.message(`${adapter.name} is paper-only. Real credentials require authority state activation.`)}>Review paper scope <ChevronRight size={14} /></Button> : <Button variant="outline" disabled={!isAuthenticated || createMutation.isPending} onClick={() => void enableSimulation(venue, adapter.scopes)}>Enable paper adapter <ChevronRight size={14} /></Button>}</footer>
      </article>;
    })}</div>
