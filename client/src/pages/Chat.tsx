  ...
  return <div className="solo-chat-page">
    ...
    {!isAuthenticated ? <section className="chat-auth-card"><LockKeyhole size={20} /><div><strong>Private conversation workspace</strong><span>Sign in to open a thread, persist the audit trail, and coordinate protected agents.</span></div><Button onClick={startLogin}>Sign in to chat</Button></section> : <section className="solo-chat-shell">
      ...
      <div className="solo-chat-conversation">
        ...
        <form className="supervisor-composer solo-composer" onSubmit={submitSupervisorMessage}><textarea value={instruction} onChange={(event) => setInstruction(event.target.value)} disabled={chatMutation.isPending} placeholder="Ask the supervisor to inspect a project, challenge a thesis, or plan a simulation-only research cycle." /><Button className="command-send" disabled={chatMutation.isPending}><Send size={14} /> Send to supervisor</Button></form>
