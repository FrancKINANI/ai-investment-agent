...
import { BrainCircuit, ClipboardCheck, LockKeyhole, RefreshCw, Send, ShieldCheck } from "lucide-react";
...
export default function Chat() {
  ...
  return <div className="solo-chat-page">
    ...
    {!isAuthenticated ? <section className="chat-auth-card"><LockKeyhole size={20} /><div><strong>Private conversation workspace</strong><span>Sign in to open a thread, persist the audit trail, and coordinate protected agents.</span></div><Button onClick={startLogin}>Sign in to chat</Button></section> : <section className="solo-chat-shell">
      <aside className="chat-thread-rail" aria-label="Conversation threads"><div><span className="eyebrow">THREADS</span><Button variant="outline" size="sm" onClick={() => setThreadId(undefined)}>New brief</Button></div><nav>{conversationsQuery.data?.map((conversation) => <button type="button" key={conversation.threadId} className={conversation.threadId === threadId ? "active" : ""} onClick={() => setThreadId(conversation.threadId)}><strong>{conversation.title}</strong><small>{new Date(conversation.updatedAt).toLocaleDateString()}</small></button>)}{!(conversationsQuery.data?.length) && <p>No thread yet. Start with a research question.</p>}</nav></aside>
      <div className="solo-chat-conversation">
        ...
        <div className="solo-chat-messages" aria-live="polite">{!threadId ? <div className="chat-empty"><BrainCircuit size={28} /><strong>Start a research conversation.</strong><span>Ask what evidence is needed, challenge a thesis, or outline a simulation-only review.</span></div> : filteredMessages.map((message) => { const presentation = getChatPresentation(message.actor, message.agentId, roleByAgentId); const confidenceBand = typeof message.confidence === "number" ? getResearchNoteConfidenceBand(message.confidence) : undefined; return <article key={message.messageId} className={`chat-message ${message.actor} ${presentation.tone}`} aria-label={`${presentation.label} message`}><header><span>{presentation.label}</span><div>{presentation.cue && <small>{presentation.cue}</small>}{confidenceBand && <b className={`confidence-score ${confidenceBand.tone}`} title="Research-note completeness based on structure, provenance cues, and stated uncertainty; it is not a forecast or return probability">Evidence completeness {message.confidence}/100 · {confidenceBand.label}</b>}</div></header><p>{message.content}</p></article>; })}{threadId && !filteredMessages.length && <div className="chat-empty"><ShieldCheck size={24} /><strong>No messages match this view.</strong><span>Choose All or wait for the delegated role to complete its bounded note.</span></div>}{chatMutation.isPending && <div className="chat-thinking"><RefreshCw size={14} className="spin" /> Supervisor is delegating bounded research tasks…</div>}</div>
        <form className="supervisor-composer solo-composer" aria-label="Supervisor message composer" onSubmit={submitSupervisorMessage}><div className="solo-composer-input"><label htmlFor="supervisor-brief">YOUR RESEARCH BRIEF <span>⌘/Ctrl + Enter to send</span></label><textarea id="supervisor-brief" value={instruction} onChange={(event) => setInstruction(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && (event.metaKey || event.ctrlKey)) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} disabled={chatMutation.isPending} placeholder="Ask the supervisor to inspect a project, challenge a thesis, or plan a simulation-only research cycle." /></div><div className="solo-composer-actions"><small>Bounded research only. No execution authority.</small><Button className="command-send" disabled={chatMutation.isPending}><Send size={15} /> <span>{chatMutation.isPending ? "Delegating…" : "Send brief"}</span></Button></div></form>
