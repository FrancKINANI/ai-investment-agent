...
export default function Chat() {
  ...
  return <div className="agent-room-page">
    ...
    {!isAuthenticated ? <><section className="agent-room-auth"><LockKeyhole size={22} /><div><strong>Open your private research desk.</strong><span>Sign in to select an agent, open protected conversations, and inspect owner-scoped memory.</span></div><Button className="mission-primary-action" onClick={startLogin}>Sign in</Button></section><section className="agent-console-locked-preview" aria-label="Agent Console layout preview"><aside><header><UsersRound size={16} /><span>AGENT ROSTER</span></header><strong>Your research team</strong><p>Your available specialist roles, status, and unread task markers appear here after owner sign-in.</p><small><LockKeyhole size={12} /> Roster is owner-scoped</small></aside><main><header><MessagesSquare size={16} /><span>FOCUSED CONVERSATION</span></header><strong>Select a specialist. Keep context visible.</strong><p>The central canvas preserves one bounded research thread for the selected agent. No agent can obtain execution authority through this conversation.</p><small><ShieldCheck size={12} /> Research context only</small></main><aside><header><BrainCircuit size={16} /><span>MEMORY INSPECTOR</span></header><strong>Shared context · private context</strong><p>The right panel distinguishes owner-approved team memory from private working memory for the selected specialist.</p><small><LockKeyhole size={12} /> Explicit scope and review</small></aside></section></> : <section className="agent-room-shell">
