...
import { Activity as ActivityIcon, CheckCheck, FileCheck2, LockKeyhole, Network, Search, ShieldCheck } from "lucide-react";
...
export default function Activity() {
  ...
  return <div className="workspace-page activity-page"><header className="workspace-heading"><span className="eyebrow">IMMUTABLE ACTIVITY LOG</span><h1>Every agent, policy, and venue event is <em>reconstructible.</em></h1><p>The log stores actual owner-scoped events only. It does not fabricate agent activity, order fills, balances, or connected accounts.</p></header>{isAuthenticated ? <section className="activity-log"><header><div className="activity-search"><Search size={15} /><input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Filter activity" /></div><div className="activity-actions"><span>{entries.length} events</span><Button variant="outline" size="sm" onClick={markAllRead} disabled={!historyQuery.data?.length}><CheckCheck size={13} /> Mark all as read</Button></div></header>{historyQuery.isLoading ? <LoadingSkeleton className="activity-loading-skeleton" label="Loading owner activity" lines={5} /> : entries.length === 0 ? <div className="log-empty"><FileCheck2 size={23} /><strong>No recorded activity.</strong><span>Run an agent cycle, save an IPS, inspect a token, audit scopes, or start a paper simulation to create the first immutable event.</span></div> : <div className="log-list">{entries.map((entry) => { const provenance = getActivityProvenance(entry.payload); return <article key={entry.actionId}><i className={`status-${entry.status}`} /><time>{new Date(entry.createdAt).toLocaleString()}</time><div><strong>{entry.subject}</strong><p>{entry.detail}</p>{provenance && <div className="activity-provenance"><span><Network size={12} /> {provenance.origin === "capability-registry" ? "Capability source" : "Owner control"}</span>{provenance.capabilities?.map((capability) => <b key={`${capability.id}-${capability.version}`}>{capability.label} <small>{capability.id} · v{capability.version} · {capability.scopes.join(" / ")}</small></b>)}<em>{provenance.registryRevision} · {provenance.executionBoundary}</em></div>}</div><span>{entry.kind.replaceAll("_", " ")}</span></article>; })}</div>}</section> : <section className="log-empty guarded"><LockKeyhole size={23} /><strong>Your audit trail is private.</strong><span>Sign in to create and inspect your agent, policy, and simulation history.</span><Button onClick={startLogin}>Sign in to view activity</Button></section>}<section className="activity-contract"><ShieldCheck size={17} /><p>Every new activity record preserves an owner-control or exact capability source, registry revision, and simulation boundary. A live order is never considered complete merely because a request was sent.</p></section></div>;
}
