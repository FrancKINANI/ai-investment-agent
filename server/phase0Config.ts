...
const systemDefaultsSchema = z.object({
  schemaVersion: z.literal(1), project: z.literal("Ledgerline"), profile: z.literal("safe-phase0"), executionBoundary: z.literal("simulation-only"),
  featureFlags: z.object({ researchSources: z.literal(true), supervisor: z.literal(true), strategyEvolution: z.literal(false), predictionMarkets: z.literal(false), cexExecution: z.literal(false), mcpActivation: z.literal(false), liveExecution: z.literal(false) }).strict(),
  ...
  orchestration: z.object({ engine: z.literal("protected-fabric"), dynamicConfiguration: z.literal(false) }).strict(),
}).strict();
const systemProfileSchema = z.object({ schemaVersion: z.literal(1), name: z.string().min(1).max(120), executionBoundary: z.literal("simulation-only"), overridePolicy: z.literal("defaults-only"), notes: z.string().min(20).max(800) }).strict();
